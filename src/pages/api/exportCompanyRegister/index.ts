...
export const GET: APIRoute = async ({ request }) => {
    ...
    const companyRef = db.collection("companyRegister");
    const companySnapshot = await companyRef.get();

    const talents = companySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Export Formulaire entreprise"); // Ajout de la feuille au classeur
    ...
    return new Response(excelBuffer, {
        ...
        headers: {
            ...
            "Content-Disposition": "attachment; filename=export-greenskills-entreprises.xls",
        },
    });
};
